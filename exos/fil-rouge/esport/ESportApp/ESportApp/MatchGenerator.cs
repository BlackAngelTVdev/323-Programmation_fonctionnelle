...
namespace ESportApp
{
    ...
    public static class MatchGenerator
    {
        ...
        public static DataSerie<DataPoint<Cs2Match>> GenerateCs2(string player, int count, int seed = 42)
        {
            ...
            return DataSerie<DataPoint<Cs2Match>>.From(
        ...
        public static DataSerie<DataPoint<ValorantMatch>> GenerateValorant(string player, int count, int seed = 42)
        {
            Random rng = new Random(seed);
            string[] agents = { "Omen", "Astra", "Brimstone" };
            DateTime start = new DateTime(2023, 9, 1);

            return DataSerie<DataPoint<ValorantMatch>>.From(
                Enumerable.Range(1, count)
                    .Select(i =>
                    {
        ...
        public static DataSerie<DataPoint<LolMatch>> GenerateLol(string player, int count, int seed = 42)
        {
            Random rng = new Random(seed);
            string[] champions = { "Thresh", "Nautilus", "Rakan", "Leona" };
            DateTime start = new DateTime(2023, 9, 1);

            return DataSerie<DataPoint<LolMatch>>.From(
                Enumerable.Range(1, count)
